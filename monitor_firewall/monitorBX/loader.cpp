...
int bpf_init(){
    ...
    // Attaching program to network interfaces
    for (int ifindex: conf.ifindices) {
        err = bpf_set_link_xdp_fd(ifindex, conf.bpf_prog_fd, XDP_FLAGS_UPDATE_IF_NOEXIST | XDP_FLAGS_SKB_MODE);
        if (err < 0) {
            fprintf(stderr, "ERROR: "
                            "ifindex(%d) link set xdp fd failed (%d): %s\n",
                    ifindex, -err, strerror(-err));
            return -2; // TODO: if I leave here, is everything cleaned up?
        }
    }
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "hf:d:s:p:")) != -1){
        switch (c){
            case 'f':
                data_file = optarg;
                break;
